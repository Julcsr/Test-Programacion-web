...
@Component
public class PageInitPaginationOffice {
	...
	public  ModelAndView initPagination(Optional<Integer> pageSize, Optional<Integer> page, String url) {
		...
		Page<Office> officesList = officeService.findAll(PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(officesList.getTotalPages(), officesList.getNumber(), BUTTONS_TO_SHOW);

		initModelView.addObject("officesList", officesList);
